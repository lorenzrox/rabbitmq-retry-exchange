...
# Build against the RabbitMQ release used by the test image.
...
RUN git clone --branch v${RABBITMQ_VERSION} https://github.com/rabbitmq/rabbitmq-server.git .

COPY . deps/rabbitmq-retry-exchange/

RUN cp rabbitmq-components.mk deps/rabbitmq-retry-exchange/rabbitmq-components.mk

RUN make RABBITMQ_VERSION=v${RABBITMQ_VERSION} -C deps/rabbitmq-retry-exchange DIST_AS_EZS=yes dist
